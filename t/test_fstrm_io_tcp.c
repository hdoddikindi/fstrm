...
static int
get_server_socket(const char *socket_addr)
{
	...
	if (sfd == -1) {
        ...
        p = strchr(socket_addr, ':');

        if (!p) {
                fprintf(stderr, "malformed endpoint address/n");
                abort();
        }

        val = ((p - socket_addr) < FSTRM_INET_LEN) ? 
            (p - socket_addr) : FSTRM_INET_LEN;

        strncpy(&buf[0], socket_addr, val);
        buf[val] = 0;
	...
	memset(&sa, 0, sizeof(sa));
        ...
        sa.sin_addr.s_addr = inet_addr(&buf[0]);
...
main(int argc, char **argv)
{
	...
	if (argc != 5) {
		fprintf(stderr, "Usage: %s <ADDRESS> <QUEUE MODEL> <NUM THREADS> <NUM MESSAGES>\n", argv[0]);
		fprintf(stderr, "\n");
		fprintf(stderr, "ADDRESS is a network endpoint.\n");
	...
	printf("testing fstrm_iothr with socket= %s "
	       ...
	       "num_threads= %u "
	       "num_messages= %u\n",
	       socket_addr, queue_model_str, num_threads, num_messages);

	struct fstrm_tcp_writer_options *twopt;
	twopt = fstrm_tcp_writer_options_init();
	fstrm_tcp_writer_options_set_socket_addr(twopt, socket_addr);
	struct fstrm_writer *w = fstrm_tcp_writer_init(twopt, NULL);
	assert(w != NULL);
	...
	test_consumer.server_fd = get_server_socket(socket_addr);
